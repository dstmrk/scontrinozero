...
| **Carryover review v1.3.0 — minor cluster** (raggruppati): (b) analytics UX polish: error banner inline + fallback `?? "Altro"` su `METHOD_LABELS` (NB: già presente come `?? e.method`, verificare se la semantica `Altro` è davvero attesa); (c) deep link `/dashboard/analytics?range=…` via searchParams (consistency con storico); (e) `onError` UI in `edit-business-section.tsx` che non discrimina retry-able vs permanent — cross-ref con P3 "Error envelope uniforme API" sopra; (g) `audit-ci.json` allowlist `GHSA-67mh-4wv8-2f99` — **motivazione documentata** sotto la tabella (vedi "Nota allowlist audit-ci").                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             | P3       |
...
| **Riuso sessione AdE — eliminare il re-login completo a ogni operazione**: ogni emissione/annullo/verifica crea un nuovo client e ripete l'intero login Fisconline (fasi A-G, ~10 round-trip HTTP **sequenziali** verso AdE) + `logout()` — `receipt-service.ts:551-555,664-668`, `void-service.ts:612-616,688`, `onboarding-actions.ts:349,604`. Il login è la **latenza dominante** (vs principio #1 performance percepita): il `submitSale` vero è un singolo POST, ma è sempre preceduto da ~10 chiamate di auth. **Design proposto:** cache in-process per-business (`Map<businessId, {client, expiresAt}>`, assunzione single-container coerente con CLAUDE.md) che conserva `RealAdeClient` (AdeSession + CookieJar) con TTL < scadenza sessione AdE (es. 10-12 min) e cap LRU; un **lock async per businessId** (catena di Promise) serializza le emissioni concorrenti così due richieste riusano un solo login invece di gareggiare (evita anche il doppio login). Il re-auth su 401 già presente in `submitDocument` resta il fallback di correttezza se la sessione è scaduta lato AdE. **Sicurezza:** la cache tiene solo i cookie di sessione in memoria (mai persistiti); le credenziali decifrate restano transienti per-operazione, **fuori** dalla cache long-lived; invalidare la entry su cambio credenziali e su `logout`. Non deve rompere l'idempotenza stale-recovery (voce backlog correlata). Escludere il path verify/onboarding (raro). Aggiungere metrica `loginCount` vs `emitCount` per misurare l'hit-rate. | P2       |
| **Rimuovere `AdeClient.getStampa()` (dead code)**: il metodo (`src/lib/ade/real-client.ts:1173`, interface `src/lib/ade/client.ts:69`, mock `src/lib/ade/mock-client.ts:92`) recupera l'HTML dello scontrino dal portale AdE ma **non ha alcun chiamante in produzione** (verificato via grep su tutto `src/`: solo le definizioni + `mock-client.test.ts`). La condivisione/stampa scontrino dopo l'emissione e dallo storico è **interamente locale**: route pubblica `src/app/r/[documentId]/pdf/route.ts` → `fetchPublicReceipt` (DB) → `generatePdfResponse` → `src/lib/pdf/generate-sale-receipt.ts` (pdfkit), senza mai toccare AdE. Rimuovere il metodo da interface/mock/real-client e il blocco `describe("getStampa")` in `mock-client.test.ts`. ⚠️ NON toccare `getProducts()` (serve alla roadmap "Sync catalogo AdE") né il flusso SPID (roadmap, vedi voce dedicata).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        | P3       |
| **`CookieJar` non onora cancellazione/scadenza dei cookie**: `src/lib/ade/cookie-jar.ts:14-33` salva solo `name=value` dal primo segmento di `Set-Cookie`, ignorando `Max-Age`/`Expires` e la semantica di **cancellazione** (`Set-Cookie: NAME=; Max-Age=0`). Se AdE invia un Set-Cookie per cancellare un cookie di sessione, il jar memorizza `NAME=""` e continua a inviarlo a ogni richiesta successiva, potenzialmente confondendo lo stato sessione del portale. Impatto basso oggi (client effimeri per-operazione), ma diventa **prerequisito** del riuso-sessione (voce sopra): un jar long-lived DEVE onorare delete/expiry. Fix: in `applyResponse` parsare `Max-Age`/`Expires`; su delete (max-age<=0, expires nel passato, o valore vuoto) fare `cookies.delete(name)` invece di memorizzare. _Da confermare con una cattura HAR che mostri un Set-Cookie di delete nel flusso di login/SSO._                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 | P3       |
| **SPID: allowlist host IdP prima del lancio** (roadmap): il flusso documenti AdE valida i redirect contro `ADE_ALLOWED_HOSTS` (`resolveAdeRedirect`, `real-client.ts:74-116`), ma il flusso SPID (`spidPostToIdp`/`spidPostCredentials`/…) segue e fa POST verso URL derivati dall'HTML del SP AdE (`parseFormAction`) e dagli header `Location` dell'IdP **senza alcuna allowlist** — e `spidPostCredentials` (`real-client.ts:759`) invia codice fiscale + password SPID dell'utente a `loginformUrl`. Se l'HTML del SP o il redirect IdP fossero manomessi (o un futuro misconfig IdP), le credenziali finirebbero su un host arbitrario (il TLS verso AdE mitiga in pratica). Poiché SPID è roadmap (non ancora cablato), implementare **insieme** al wiring di `loginSpid` una allowlist di hostname IdP noti (es. `identity.sieltecloud.it` + altri provider SPID) analoga a `ADE_ALLOWED_HOSTS`. _Da confermare al momento dell'attivazione SPID._                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | P3       |
| **Documentare il flusso SPID come "roadmap, non cablato"**: `loginSpid`/`authenticateSpid` + helper S1-S15 + helper legacy Liferay usati solo da SPID (`extractPAuth`/`initDataPowerSession`/`activateSession`/`verifySession`) + tipo `SpidCredentials` + `AdeSpidTimeoutError` + opzioni `spidPollIntervalMs`/`spidMaxPolls` (~600 righe di `real-client.ts`) **non hanno alcun chiamante in produzione** (solo il login Fisconline è cablato). È codice voluto per il futuro: aggiungere un commento-header esplicito in `real-client.ts` (e/o nota nella sezione roadmap di PLAN.md) che lo marca "in attesa di lancio SPID", così non venga scambiato per dead-code in review future; assicurarsi che resti coperto dai test (o escluso da coverage in modo tracciato).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | P3       |

### Nota allowlist audit-ci (`GHSA-67mh-4wv8-2f99`)
