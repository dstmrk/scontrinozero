...
| ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **v1.4.x**  | _Shipped._ Supporto in-app: voce "Supporto" nelle impostazioni del dashboard (link a `/help/contatto-assistenza`, mailto pre-compilato). + Visibilità in-app dell'annullamento a fine periodo (`cancel_at_period_end`): la card billing distingue "in cancellazione, attivo fino al <data>" (ex REVIEW.md #34). |
| **v1.4.1**  | **Onboarding tour dashboard al primo accesso**: walkthrough guidato delle funzioni chiave (emissione scontrino, catalogo, storico, impostazioni), mostrato una sola volta e skippabile, con flag "tour visto" persistito per-utente (da decidere: `localStorage` vs colonna su `profiles`). Libreria scelta: **`react-joyride` v3** (React 19 dalla v3 di marzo 2026) per coerenza con shadcn/ui — il suo `tooltipComponent` rende i nostri componenti `src/components/ui` (`Card`/`Button`/`Popover`), quindi token OKLCH, dark mode e a11y/spotlight gratis; **caricata in `dynamic import`** (componente client-only, dashboard-only, mostrato una volta) così i ~34 KB stanno in un chunk separato e non toccano il bundle principale né la performance percepita. Trade-off accettato: coupling alla versione React (rotto su React 19 fino alla v3 → attenzione ai futuri upgrade Next/React). Fallback `driver.js` (~5 KB, vanilla, zero coupling) se il coupling diventa un problema; `intro.js` escluso (AGPL). Mobile-first/PWA. Da approfondire in conversazione ad-hoc prima dell'implementazione. |
