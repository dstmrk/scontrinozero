...
| **v1.4.2**  | **Onboarding tour dashboard al primo accesso**: walkthrough guidato delle funzioni chiave (emissione scontrino, catalogo, storico, impostazioni), mostrato una sola volta e skippabile, con flag "tour visto" persistito per-utente (da decidere: `localStorage` vs colonna su `profiles`). Libreria candidata: **driver.js** (~5 KB, zero dipendenze, MIT, vanilla → nessun coupling con la versione React/Next, coerente con "leggeri sulle risorse"); alternativa React-native `react-joyride` v3 (~34 KB, dalla v3 di marzo 2026 supporta React 19). `intro.js` escluso (AGPL). Mobile-first/PWA. Da approfondire in conversazione ad-hoc prima dell'implementazione. |
