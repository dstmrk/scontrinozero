...
## Versione corrente: v1.2.5 ⬜ — Prossima release: v1.2.6 (fix code review indipendente)
...
| **v1.2.6**   | ⬜ Fix code review indipendente: webhook dedup correctness (SELECT-then-INSERT), plan/trial enforcement su emit/void, Zod validation server action, date UTC normalization, Invalid Date guard API |
...
| **B9**  | **Recovery/replay eventi Stripe in stato `failed`** (da code review P0-01): con il pattern SELECT-then-INSERT gli eventi falliti non vengono marcati come processati e Stripe li riemette automaticamente. Per aumentare la robustezza, aggiungere un job periodico che rileva eventi rimasti "stuck" (mai inseriti in `stripe_webhook_events` dopo N tentativi falliti) e li reinvia al handler. Richiede infrastruttura background task (es. cron sul container).                                                                                                                         | P3       |
| **B10** | **`rows_affected >= 1` check sui webhook handler critici** (da code review P1-06): nei handler `invoice.paid`, `invoice.payment_action_required`, `invoice.payment_failed`, `checkout.session.expired` non viene verificato che l'`UPDATE` abbia effettivamente aggiornato righe. Un match mancante viene attualmente loggato in modo silenzioso (o non loggato affatto) e l'evento viene marcato come processato. Aggiungere controllo esplicito su `rowsAffected` e trattare come errore se 0, così Stripe può ritentare. Da implementare contestualmente a B4 (error envelope uniforme). | P2       |
