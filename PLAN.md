...
| ---------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **v1.6.0** | **Stampa scontrino su termica Bluetooth** (Web Bluetooth, Android) **+ fallback PDF** ovunque. Un solo bottone "Stampa": dove il browser espone Web Bluetooth (Chrome/Edge/Samsung Internet su Android) stampa direttamente in ESC/POS sulla stampantina termica; altrove apre il PDF **già generato a 58mm** (`src/lib/pdf/generate-sale-receipt.ts` via `src/app/api/documents/[documentId]/pdf/route.ts`) nel foglio di stampa OS. Nessun plan-gate. _Il foglio di stampa OS da solo (AirPrint/Mopria) parla **solo rete/WiFi** e non raggiunge le stampantine BT ESC/POS da ~50 €, che sono il parco reale dei micro-esercenti: spedire solo quello avrebbe dato una feature inutilizzabile alla maggioranza. Web Bluetooth le raggiunge oggi, senza wrapper nativo. La corsia PDF resta come fallback su iOS/Firefox/desktop, così nessun utente resta senza stampa. Moduli: `src/lib/printing/`, `src/hooks/use-printer.ts`, `src/components/printing/`._ |
| **v1.7.0** | **Storno avanzato**: memorizzare progressivo documento AdE di annullamento e stampare ricevuta di annullamento|
