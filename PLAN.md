...
## Versione corrente: v1.2.7 ✅ — Prossima release: v1.2.8 (Hardening review fixes + SEO Foundations)
...
- ⬜ Screenshot UI dell'app (mockup mobile) sulla home, `/funzionalita`, landing categoria
  (debito tecnico ereditato da v1.2.3): `next/image` con `priority` per LCP, `<img>` `alt`
  descrittive con keyword
...
| **B13** | **DB query timeouts e circuit breaker applicativo** (da code review P1-02): le route critiche (`/api/v1/receipts`, endpoint PDF, auth actions) non hanno timeout/budget di latenza espliciti. Saturazione DB o lock anomali possono propagare timeout a cascata. Fix: wrapper con `AbortSignal.timeout` o timeout a livello pool postgres-js, mappare timeout a 503 retryable distinto dagli errori logici, contatori su durata e timeout. Richiede design dedicato (pool config + error envelope coerente con B4).                                             | P1       |
| **B14** | **CSP completa con rollout report-only → enforce** (da code review P0-01): in v1.2.8 sono stati introdotti i baseline security headers (`X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, `Permissions-Policy`, HSTS in prod). Manca la `Content-Security-Policy`. Mappare le origini di Sentry, Turnstile, Stripe, Resend, Supabase, font, Umami, eventuali asset di terze parti; pubblicare la CSP in `Content-Security-Policy-Report-Only` per almeno una release con endpoint di report; promuovere a enforce dopo zero violation reali.      | P1       |
