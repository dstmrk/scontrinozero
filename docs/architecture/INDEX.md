...
| Resa del documento commerciale (layout AdE)       | Tre superfici, stesso layout: PDF `src/lib/pdf/commercial-document.ts` · termica `src/lib/printing/receipt-escpos.ts` · pagina pubblica `src/app/r`. Diciture e indirizzo condivisi in `src/lib/receipt-format.ts` (`PAYMENT_LABELS`, `formatBusinessAddressLines`), codifica IVA/legenda in `src/lib/receipts/vat-display.ts` (`receiptVatLabel` → `ES*`, `receiptVatLegend` → `*ES = Esente`) |
