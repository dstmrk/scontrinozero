...
| Data fiscale del documento                        | `ade_registered_at` (migrazione 0031): non solo la data **mostrata** su ogni superficie, ma anche il **predicato di periodo** e l'ordinamento di elenco storico (`src/server/storico-actions.ts`) ed export CSV (`src/lib/receipts/csv-export.ts`) — indice `(business_id, ade_registered_at)`, migrazione 0032. `created_at` resta il nostro orologio d'inserimento, usato dal recovery stale-pending. Gli estremi dell'intervallo sono ancora mezzanotte UTC, non italiana (REVIEW #94)                                                                                                                                    |
