...
src/
  app/            App Router. Route group + route handler API.
    ...
    sitemap.ts robots.ts manifest.ts layout.tsx error.tsx global-error.tsx
...
`src/app/api/export`, `src/app/api/csp-report`, `src/app/api/auth`
(Send Email Hook di Supabase Auth).
...
| Cerchi…                                               | Vai a                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| ----------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Gate del pannello operatore (`/admin`)                | `src/lib/admin-gate.ts` (`isAdminEmail`, allowlist `ADMIN_EMAILS`), applicato in `src/app/admin/layout.tsx` — 404, non 403. Letture in `src/server/admin-metrics.ts` (KPI) e `src/server/admin-directory.ts` (elenchi con dati personali); `src/server/admin-sql.ts` traduce in SQL il canone centesimi-per-riga di `src/lib/receipts/receipt-totals.ts` e impone il tetto di **una** connessione per volta (`runAdminRead`)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| Errore non gestito di una pagina (boundary)           | contenuto condiviso `src/components/errors/app-error-fallback.tsx` (cattura in Sentry i soli errori client, recupero dal deploy skew) · root `src/app/error.tsx` (copre onboarding, auth, marketing, `/r`) · segmento dashboard `src/app/dashboard/error.tsx` (tiene shell e bottom nav) · fallback globale `src/app/global-error.tsx` (rimpiazza il documento intero; resta per gli errori del root layout) · degrado inline del piano `src/components/dashboard/plan-unavailable.tsx`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| Pagamenti, lotteria e sconto a pagare di un documento | `src/lib/receipts/public-request.ts` — unico lettore del jsonb `commercial_documents.public_request`, **puro e client-safe**. `parsePublicRequest` normalizza il blob (righe storiche comprese: sono `NULL` o portano il solo `paymentMethod`); `resolvePaymentRows` ricompone il blocco pagamenti in voci `{type, amountCents}` **sia** sui pagamenti misti **sia** sui metodi singoli, ed è il modo corretto di leggerlo — `paymentMethod` da solo non descrive un misto. Consumato da storico, ricevuta pubblica, PDF, termica, export CSV, analytics e `/api/v1`; `src/lib/receipts/v1-payments.ts` lo rende nella forma del contratto pubblico                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| Pagamento in ingresso (cassa + Developer API)         | `src/lib/receipts/payment-input.ts` — speculare a `public-request.ts` ma sul lato input: `toPaymentEntries` porta gli importi in euro alla stessa forma canonica in centesimi, `isMixedPayment` è il predicato del gate Pro e della lotteria. La quadratura `Σ importi + sconto a pagare = totale righe` (`HAR.md` voce #5) la impone `refinePaymentDeclaration` in `receipt-schema.ts`, in centesimi interi; i gate di piano stanno in `src/lib/receipts/pro-feature-gates.ts` (sconti + pagamento misto), applicati da `receipt-actions.ts` e da `POST /api/v1/receipts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| Resa del documento commerciale (layout AdE)           | Tre superfici, stesso layout, **due forme** (vendita e annullo, unione discriminata su `kind`): PDF `src/lib/pdf/commercial-document.ts` · termica `src/lib/printing/receipt-escpos.ts` · pagina pubblica `src/app/r`. Diciture e indirizzo condivisi in `src/lib/receipt-format.ts` (`PAYMENT_LABELS`, `formatBusinessAddressLines`, `formatReceiptDate`), codifica IVA/legenda in `src/lib/receipts/vat-display.ts` (`receiptVatLabel` → `ES*`, `receiptVatLegend` → `*ES = Esente`). Chi è stampabile lo decide `src/lib/receipts/printable-document.ts`; la data stampata è sempre `ade_registered_at`, mai `created_at`. Il messaggio di cortesia in coda (Pro) è `businesses.receipt_footer_note`, reso da `receiptFooterNoteLines` e risolto dal gate di piano nei lettori (`src/lib/receipts/footer-note.ts`) — solo sulla vendita, mai su un annullo                                                                                                                                                                                                                                                                                                  |
| Documenti che vivono solo su AdE (Pro, v1.8.0)        | Emessi fuori da ScontrinoZero — dal portale o dall'app AdE, o da altro software che usa lo stesso servizio. **Sola lettura, mai copiati nel DB.** Catena: `src/lib/services/ade-user-session.ts` (`resolveAdeUserSession`, condiviso con la verifica dei PENDING) → `src/lib/services/ade-document-search.ts` (`buildAdeSearchRanges` spezza il periodo in una query per mese solare — `ADE_QUERY_MAX_DAYS` = 31 è il vincolo del portale su UNA query, `ADE_SEARCH_MAX_DAYS` = 366 è quanto l'esercente può chiedere; `fetchAdeSaleRows` le esegue in sequenza dalla più recente, con un deadline che dichiara `truncated` invece di farsi chiudere dal proxy) → `src/lib/services/ade-storico-rows.ts` (`fetchForeignAdeRows`: filtro, deduplica su `idtrx` via `findClaimedTransactionIds`, degrado a `adeError`). I lettori sono **due** — `searchReceiptsIncludingAde` e `buildReceiptsCsvStream` — e devono vedere le stesse righe nello stesso ordine, che è `compareStoricoOrder` in `src/lib/receipts/storico-order.ts`. Lo stato di una riga si deriva dal flag `annulli` della vendita (`HAR.md` #16c), quindi si interroga solo `tipoOperazione=V` |
...
| Recovery stale-pending AdE | `src/lib/services/ade-recovery.ts`, `src/lib/services/request-hash.ts`; verifica manuale dentro la sessione esercente `src/lib/services/pending-verification.ts` + `src/server/pending-actions.ts` + banner `src/components/dashboard/pending-sales-banner.tsx`; rilevatore su `/admin` e sweep in `src/instrumentation.ts` |
...
| CSP / security headers | `src/lib/csp.ts`, `src/lib/security-headers.ts` (statici), `src/proxy.ts` (quelli che dipendono dall'host) |
...
| Mail di conferma registrazione | `src/app/api/auth/send-email` (Send Email Hook di GoTrue) + template `src/emails/confirm-signup.tsx`. Firma Standard Webhooks in `src/lib/standard-webhook.ts`. Il reset password NON passa di qui: `resetPassword` usa `admin.generateLink` + `sendEmail` |
...
"degradare, non lanciare" (CLAUDE.md regola 19). La tabella elenca anche i tre
moduli di `src/server/` che **non** sono server action — `admin-metrics.ts`,
`admin-directory.ts` e `admin-sql.ts`: il suffisso diverso è deliberato, non
espongono alcun endpoint RPC e la loro unica via d'accesso è la RSC `/admin`
dietro il gate del layout.
...
| `src/server/storico-actions.ts`    | elenco/ricerca documenti emessi + rilettura di una singola riga. `searchReceipts` legge il solo DB; `searchReceiptsIncludingAde` (Pro) fonde in memoria anche l'archivio AdE — vedi la riga "Documenti che vivono solo su AdE" nella tabella qui sopra                                                                                      |
| `src/server/analytics-actions.ts`  | KPI e analytics (helper in `src/server/analytics-helpers.ts`)                                                                                                                                                                                                                                                                               |
| `src/server/admin-metrics.ts`      | KPI del pannello operatore su tutti i tenant, in **due letture separate** (`getAdminUserKpis` su `profiles`, `getAdminDocumentKpis` su `commercial_documents`) — **NON** server action (nessun `"use server"`, quindi nessun endpoint RPC): le chiama solo la RSC `/admin`, dietro il gate del layout                                       |
| `src/server/admin-directory.ts`    | Elenchi del pannello operatore, in **quattro letture separate**: `getAdminTopMerchants` e `getAdminRecentProfiles` prendono un range, `getAdminTrialExpiring` e `getAdminPaidUsers` sono ancorate ad adesso e non ne prendono. Separato da `admin-metrics.ts` perché ogni riga porta **nome ed email**: niente di tutto ciò finisce nei log |
| `src/server/admin-sql.ts`          | Primitive condivise dalle sei letture admin: `runAdminRead` (tetto di **una** connessione per volta + budget di timeout), `adminRangeParams`, coercizioni dai risultati grezzi di postgres-js, e `lineCentsSql` — il gemello SQL del canone monetario                                                                                       |
