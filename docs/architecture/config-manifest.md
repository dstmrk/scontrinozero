...
| Soglie GDPR pruning utenti inattivi (enabled/giorni preavviso+delete)    | `src/lib/services/inactive-user-prune-config.ts`                                          | Env `INACTIVE_USER_PRUNE_*` opt-in, runtime; cadenza sweep fissa 24h in `src/instrumentation.ts`       |
